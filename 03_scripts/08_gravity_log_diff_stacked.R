...
est = mutate(est, estimator = "zip", sample = "stacked", spec = "raw") %>% 
  as.data.frame()
...
write_rds(est, file = "02_gen/04_results/zip_pooled_stacked_log_diff.rds")
...
#### Add the estimator and sample ####
est = mutate(est, estimator = "zip", sample = "stacked", spec = "gravity") %>% 
  as.data.frame()
...
#### Save the Poisson plot ####
write_rds(est, file = "02_gen/04_results/zip_stacked_gravity_log_diff.rds")
...
est <- lapply(est_twfe, bind_rows) %>%
  bind_rows(., .id = "var") %>% 
  mutate(specification = "twfe_model")
...
est <- left_join(est, stat_imb, by = "var")
...
est <- left_join(est, stat_dep, by = "var")
...
est <- left_join(est, stat_period_pairs, by = "var")

#### Save the Poisson plot ####
write_rds(est, file = "02_gen/04_results/zip_stacked_twfe_log_difference.rds")

#### --------------------------------------------------------------------- #### 
#### Trends model  ZIP for log diferences ####
#### --------------------------------------------------------------------- ####
...
#### Demeaned the data by country pair, year, and year-country ####
pair_var_means <- data %>%
  group_by(pair, var, prod_class) %>%
  summarise(pair_var_mean = mean(volume, na.rm = TRUE), .groups = "drop")
...
from_year_var_means <- data %>%
  group_by(from, year, var, prod_class) %>%
  summarise(from_year_var_mean = mean(volume, na.rm = TRUE), .groups = "drop")

to_year_var_means <- data %>%
  group_by(to, year, var, prod_class) %>%
  summarise(to_year_var_mean = mean(volume, na.rm = TRUE), .groups = "drop")

data <- data %>%
  left_join(pair_var_means,       by = c("pair", "var")) %>%
  left_join(year_var_means,       by = c("year", "var")) %>%
  left_join(from_year_var_means,  by = c("from", "year", "var")) %>%
  left_join(to_year_var_means,    by = c("to", "year", "var")) %>%
  
  mutate(demeaned_b = volume - pair_var_mean - year_var_mean - 
           from_year_var_mean - to_year_var_mean) %>% group_by(var) %>%
  # shift everything so it's positive before converting to integer
  mutate(demeaned_b = as.integer(demeaned_b + abs(min(demeaned_b)))) %>%
  ungroup()
...
#### Run the TWFEs model ####
est_trend <- lapply(data_split, function(x) {
  fn(
    data      = x,
    formula   = demeaned_a ~ imbalance,
    ziformula = ~ .,
...
#### Bind the estimates together
est <- lapply(est_trend, bind_rows) %>%
  bind_rows(., .id = "var") %>% 
  mutate(specification = "trend_model")
...
#### Save the Poisson plot ####
write_rds(est, file = "02_gen/04_results/zip_stacked_trend_log_difference.rds")
